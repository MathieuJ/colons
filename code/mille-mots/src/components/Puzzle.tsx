...
import { WORD_LENGTH } from "../util/words";
    ...
    {!expired &&
      <Row>
        {working.split('').map((letter, i) => (
          <Letter red={working.length === WORD_LENGTH && !checkValidity(working)} key={`${letter}${i}W`}>{letter}</Letter>
        ))}
        {working.length <= WORD_LENGTH && Array.from(Array(WORD_LENGTH - working.length)).map((e, i) => <Letter key={`empty-${i}`}> </Letter>)}
      </Row>
    }
  </Container>
  );
