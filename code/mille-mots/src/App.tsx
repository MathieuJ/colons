...
function App() {
  ...
  const maxGuesses = NUMBER_WORDS + 5;
  ...
  function addKey(key: string) {
    ...
    } else if (working.length !== WORD_LENGTH && key.length === 1 && key !== ' ') {
  ...
  useEffect(() => {
    ...
    if (!expired) {
      ...
      if (working.length > WORD_LENGTH) setWorking((tmp) => tmp.slice(0, WORD_LENGTH));
