...
app.controller('CodenamesCtrl', function($scope, $location) {
 ...
 function myRandom(seed) {
  ...
  $scope.generateMaitre = function() {
    $scope.graineEspion = getGraineEspion($scope.graineMaitre);
    build();
    $scope.demarre = true;
    $scope.generate();
  }
  ...
  function build() {
    ...
    var tableau = [];
      ...
      $scope.sousgroupe = _.map($scope.sousgroupe, function(m) { return { mot : m, style : "rien" }; });
  ...
  $scope.modifyStyle = function(cell) {
    switch (cell.style) {
      case 'rien' : cell.style = 'rouge'; break;
      case 'rouge' : cell.style = 'bleu'; break;
      case 'bleu' : cell.style = 'marron'; break;
      case 'marron' : cell.style = 'noir'; break;
      default : cell.style = 'rien'; break;
    }
  }
});
